...
- Release/versioning files when needed, but keep `CHANGELOG.md` under the release automation instead of editing it manually
...
- Conventional Commits are used for automatic version calculation via release-it, but non-conventional commits must not block CI.
- The `release.yml` workflow runs after CI passes on `main`, using release-it to bump version, update CHANGELOG.md, create and push a Git tag.
- The `publish.yml` workflow runs on tag push and publishes to npm with provenance.
- `CHANGELOG.md` is generated output managed by release-it with the conventional-changelog plugin; do not hand-edit.
- Commitlint is not used in CI to ensure non-conventional commits do not block the workflow.
...
- `n8n-node cloud-support` - manage n8n Cloud eligibility.
  ...
  cloud eligibility)
- `npm run release` - run `release-it` for the conventional-commit release
  flow in this repository.
  - Updates the version and changelog
  - Creates the release commit and git tag
  - Pushes the tag to GitHub, triggering the `publish.yml` workflow
  - Does **not** publish to npm directly; the tag-triggered `publish.yml`
    workflow handles npm publishing with provenance
